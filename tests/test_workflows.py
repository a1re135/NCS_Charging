...
class WorkflowTests(unittest.TestCase):
    ...
    def test_charger_maintenance_and_offline_status(self):
        a,t=self.login('admin','Admin123456')
        for op in ('maintenance','offline'):
            self.assertEqual(self.post('/admin/chargers/1/action',{'action':op},a,t).status_code,200)
            self.assertEqual(self.sql('SELECT status FROM chargers WHERE id=1')[0][0],op)
            self.assertEqual(self.post('/orders',{'charger_id':1,'mode':'start'}).status_code,409)
        self.assertEqual(self.post('/admin/chargers/1/action',{'action':'restore'},a,t).status_code,200)
        self.assertEqual(self.sql('SELECT status FROM chargers WHERE id=1')[0][0],'idle')
        self.assertEqual(self.post('/admin/chargers/1/action',{'action':'bad-op'},a,t).status_code,400)
    def test_station_filters_and_usage_sort(self):
        r=self.client.get('/api/stations?status=idle');self.assertEqual(r.status_code,200);self.assertTrue(r.json)
        for st in r.json: self.assertGreater(st['free'],0)
        r=self.client.get('/api/stations?status=maintenance');self.assertTrue(r.json)
        for st in r.json: self.assertGreater(st['maintenance'],0)
        r=self.client.get('/api/stations?status=offline');self.assertTrue(r.json)
        for st in r.json: self.assertGreater(st['offline'],0)
        r=self.client.get('/api/stations?kind=fast');self.assertTrue(all(st['fast']>0 for st in r.json))
        r=self.client.get('/api/stations?kind=slow');self.assertTrue(all(st['slow']>0 for st in r.json))
        r=self.client.get('/api/stations?sort=usage');usages=[st['usage'] for st in r.json]
        self.assertEqual(usages,sorted(usages,reverse=True))
        for bad in ('status=bad','kind=bad','sort=bad'):
            self.assertEqual(self.client.get('/api/stations?'+bad).status_code,400)
    def test_order_date_filter_and_export(self):
        self.start()
        today=datetime.now().strftime('%Y-%m-%d')
        r=self.client.get('/api/orders?date_from='+today+'&date_to='+today)
        self.assertEqual(r.status_code,200)
        for o in r.json: self.assertEqual(o['created_at'][:10],today)
        self.assertEqual(self.client.get('/api/orders?date_from=2000-01-01&date_to=2000-01-02').json,[])
        self.assertEqual(self.client.get('/api/orders?date_from=bad').status_code,400)
        self.assertEqual(self.client.get('/api/orders?date_from=2026-01-02&date_to=2026-01-01').status_code,400)
        a,t=self.login('admin','Admin123456')
        r=a.get('/api/admin/export?date_from='+today+'&date_to='+today);self.assertEqual(r.status_code,200)
        self.assertTrue(r.data.startswith(b'\xef\xbb\xbf'))
    def test_admin_user_status_filters_and_sort(self):
        a,t=self.login('admin','Admin123456')
        r=a.get('/api/admin/users');self.assertEqual(r.status_code,200)
        u3=next(u for u in r.json if u['id']==3)
        self.assertGreater(u3['debt_cents'],0)
        r=a.get('/api/admin/users?status=debt')
        for u in r.json: self.assertGreater(u['debt_cents'],0)
        r=a.get('/api/admin/users?status=normal')
        for u in r.json: self.assertEqual(u['debt_cents'],0);self.assertEqual(u['active'],1)
        self.post('/admin/users/1',{'active':False},a,t)
        r=a.get('/api/admin/users?status=frozen');self.assertTrue(any(u['id']==1 for u in r.json))
        self.sql('UPDATE users SET created_at=? WHERE id=3',('2025-06-01T10:00:00',))
        r=a.get('/api/admin/users?sort=oldest');self.assertEqual(r.json[0]['id'],3)
        r=a.get('/api/admin/users?sort=newest');self.assertNotEqual(r.json[0]['id'],3)
        r=a.get('/api/admin/users?date_from=2099-01-01&date_to=2099-12-31');self.assertEqual(r.json,[])
        self.assertEqual(a.get('/api/admin/users?status=bad').status_code,400)
    ...
    def test_station_kind_breakdown_fields(self):
        for st in self.client.get('/api/stations?lat=39.9593&lng=116.2981').json:
            for f in ('fast_free','slow_free','fast_fault','slow_fault','fast_maintenance','slow_maintenance','fast_offline','slow_offline'):
                self.assertIn(f,st)
                self.assertGreaterEqual(st[f],0)
            self.assertLessEqual(st['fast_free'],st['fast'])
            self.assertLessEqual(st['slow_free'],st['slow'])
            self.assertEqual(st['fast_free']+st['slow_free'],st['free'])

    def test_order_trend_endpoint(self):
        oid=self.start();self.age(oid,600);self.assertEqual(self.post(f'/orders/{oid}/finish').status_code,200)
        a,t=self.login('admin','Admin123456')
        r=a.get('/api/admin/trend');self.assertEqual(r.status_code,200)
        p=r.json['points'];self.assertGreaterEqual(len(p),7)
        for x in p:
            self.assertGreaterEqual(x['done'],0)
            self.assertLessEqual(x['done'],x['total'])
            self.assertGreaterEqual(x['cents'],0)
        self.assertEqual(a.get('/api/admin/trend?granularity=week&range=30').status_code,200)
        self.assertEqual(a.get('/api/admin/trend?granularity=month&range=year').status_code,200)
        self.assertEqual(a.get('/api/admin/trend?granularity=day&range=7&station_id=1').status_code,200)
        self.assertEqual(a.get('/api/admin/trend?granularity=bad').status_code,400)
        self.assertEqual(a.get('/api/admin/trend?range=bad').status_code,400)
        self.assertEqual(a.get('/api/admin/trend?station_id=999').status_code,404)
        c2,t2=self.login('13900139000','User123456')
        self.assertEqual(c2.get('/api/admin/trend').status_code,403)

    def test_revenue_stations_endpoint(self):
        oid=self.start();self.age(oid,600);self.assertEqual(self.post(f'/orders/{oid}/finish').status_code,200)
        a,t=self.login('admin','Admin123456')
        r=a.get('/api/admin/revenue_stations');self.assertEqual(r.status_code,200)
        st=r.json['stations'];self.assertEqual(len(st),10)
        rev=[x['revenue_cents'] for x in st]
        self.assertEqual(rev,sorted(rev,reverse=True))
        self.assertGreater(rev[0],0)
        self.assertGreaterEqual(r.json['average_cents'],0)
        self.assertIn('debt_cents',st[0])
        c2,t2=self.login('13900139000','User123456')
        self.assertEqual(c2.get('/api/admin/revenue_stations').status_code,403)
